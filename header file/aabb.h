...
private:
    void padToMinimums() {
        ...
        double delta = 0.0001;
        if (intervalX.getSize() < delta) intervalX = intervalX.expand(delta);
        if (intervalY.getSize() < delta) intervalY = intervalY.expand(delta);
        if (intervalZ.getSize() < delta) intervalZ = intervalZ.expand(delta);
    }
};
